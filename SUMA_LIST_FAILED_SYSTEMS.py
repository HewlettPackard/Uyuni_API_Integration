...
# Purpose: Returns a list of systems that have failed a specific action.
...
# Script Name: SUMA_LIST_FAILED_SYSTEMS.py
...
def connect_suma(mgr_login = '', mgr_pass = ''):
    ...
    return client, key
def list_failed_systems(client, key, aid):
    try:
        res = client.schedule.listFailedSystems(key, aid)
        #print (res)
        if res is None:
...
def main():
    try:
        ...
        for aid in action_ids:
            #print(aid)
            response = list_failed_systems(client, key, int(aid))
            if response is None:
                print("INFO ! No system is listed for action ID: %s" % aid)
            else:
                #print(response)
        ...
        print("ERROR! Check script SUMA_LIST_FAILED_SYSTEMS . Can't Fetch list of Failed Systems.")
